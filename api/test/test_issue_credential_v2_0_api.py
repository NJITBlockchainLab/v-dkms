# coding: utf-8

"""
    sp

    No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)  # noqa: E501

    OpenAPI spec version: v0.6.0
    
    Generated by: https://github.com/swagger-api/swagger-codegen.git
"""


from __future__ import absolute_import

import unittest

import swagger_client
from swagger_client.api.issue_credential_v2_0_api import IssueCredentialV20Api  # noqa: E501
from swagger_client.rest import ApiException


class TestIssueCredentialV20Api(unittest.TestCase):
    """IssueCredentialV20Api unit test stubs"""

    def setUp(self):
        self.api = swagger_client.api.issue_credential_v2_0_api.IssueCredentialV20Api()  # noqa: E501

    def tearDown(self):
        pass

    def test_issue_credential20_create_post(self):
        """Test case for issue_credential20_create_post

        Send holder a credential, automating entire flow  # noqa: E501
        """
        pass

    def test_issue_credential20_records_cred_ex_id_delete(self):
        """Test case for issue_credential20_records_cred_ex_id_delete

        Remove an existing credential exchange record  # noqa: E501
        """
        pass

    def test_issue_credential20_records_cred_ex_id_get(self):
        """Test case for issue_credential20_records_cred_ex_id_get

        Fetch a single credential exchange record  # noqa: E501
        """
        pass

    def test_issue_credential20_records_cred_ex_id_issue_post(self):
        """Test case for issue_credential20_records_cred_ex_id_issue_post

        Send holder a credential  # noqa: E501
        """
        pass

    def test_issue_credential20_records_cred_ex_id_problem_report_post(self):
        """Test case for issue_credential20_records_cred_ex_id_problem_report_post

        Send a problem report for credential exchange  # noqa: E501
        """
        pass

    def test_issue_credential20_records_cred_ex_id_send_offer_post(self):
        """Test case for issue_credential20_records_cred_ex_id_send_offer_post

        Send holder a credential offer in reference to a proposal with preview  # noqa: E501
        """
        pass

    def test_issue_credential20_records_cred_ex_id_send_request_post(self):
        """Test case for issue_credential20_records_cred_ex_id_send_request_post

        Send issuer a credential request  # noqa: E501
        """
        pass

    def test_issue_credential20_records_cred_ex_id_store_post(self):
        """Test case for issue_credential20_records_cred_ex_id_store_post

        Store a received credential  # noqa: E501
        """
        pass

    def test_issue_credential20_records_get(self):
        """Test case for issue_credential20_records_get

        Fetch all credential exchange records  # noqa: E501
        """
        pass

    def test_issue_credential20_send_offer_post(self):
        """Test case for issue_credential20_send_offer_post

        Send holder a credential offer, independent of any proposal  # noqa: E501
        """
        pass

    def test_issue_credential20_send_post(self):
        """Test case for issue_credential20_send_post

        Send holder a credential, automating entire flow  # noqa: E501
        """
        pass

    def test_issue_credential20_send_proposal_post(self):
        """Test case for issue_credential20_send_proposal_post

        Send issuer a credential proposal  # noqa: E501
        """
        pass


if __name__ == '__main__':
    unittest.main()
