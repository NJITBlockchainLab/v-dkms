...
Feature: RFC 0160 Aries agent connection functions
   ...
   Scenario Outline: establish a connection between two agents
      Given we have "2" agents
         ...
         | DMV   | inviter |
         | Bob   | invitee |
      When "DMV" generates a connection invitation
      And "Bob" receives the connection invitation
      And "Bob" sends a connection request to "DMV"
      And "DMV" receives the connection request
      And "DMV" sends a connection response to "Bob"
      And "Bob" receives the connection response
      And "DMV" sends <message> to "Bob"
   ...
   Scenario: Inviter Sends invitation for one agent second agent tries after connection
      ...
      And "DMV" generated a single-use connection invitation
      And "Bob" received the connection invitation
      And "Bob" sent a connection request to "DMV"
      And "DMV" receives the connection request
      And "DMV" sends a connection response to "Bob"
      And "Bob" receives the connection response
      And "DMV" and "Bob" have a connection
      When "Mallory" sends a connection request to "DMV" based on the connection invitation
      Then "DMV" sends a request_not_accepted error
   ...
   Scenario: Inviter Sends invitation for multiple agents
      Given we have "3" agents
         ...
         | Bob     | invitee           |
         | Mallory | inviteinterceptor |
      And "DMV" generated a multi-use connection invitation
      And "Bob" receives the connection invitation
      And "Bob" sends a connection request to "DMV"
      And "DMV" receives the connection request
      And "DMV" sends a connection response to "Bob"
      When "Mallory" sends a connection request to "DMV" based on the connection invitation
      Then "DMV" sends a connection response to "Mallory"
   #And "DMV" and "Bob" are able to complete the connection
   #And "DMV" and "Mallory" are able to complete the connection
   ...
   Scenario: Establish a connection between two agents who already have a connection initiated from invitee
      ...
      And "Bob" and "DMV" complete the connection process
      Then "DMV" and "Bob" have another connection
   ...
   Scenario Outline: Establish a connection between two agents but gets a request not accepted report problem message
      ...
      And "Bob" receives the connection invitation
      And "Bob" sends a connection request to "DMV"
      Then "DMV" sends an request not accepted error
      And the state of "DMV" is reset to Null
